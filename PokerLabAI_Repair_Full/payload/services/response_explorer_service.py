...
class ResponseExplorerService:
    ...
    BUCKET_ORDER = CoreAnalyticsEngine.BUCKET_ORDER
    ...
    def _load_rows(
        ...
        query = f"""
            ...
            street_players AS (
                SELECT
                    ...
                    MAX(
                        CASE
                            WHEN UPPER(TRIM(a.action)) = 'CALL'
                            THEN 1 ELSE 0
                        END
                    ) AS called,
                    MAX(
                        CASE
                            WHEN UPPER(TRIM(a.action)) = 'RAISE'
                            THEN 1 ELSE 0
                        END
                    ) AS raised
                FROM actions a
        ...
        columns = [
            ...
            "turn_fold_vs_barrel",
            "turn_call_vs_barrel",
            "turn_raise_vs_barrel",
            "river_barrel_made",
            "river_fold_vs_barrel",
            "river_call_vs_barrel",
            "river_raise_vs_barrel",
        ]
        ...
        for raw in raw_rows:
            ...
            row["position"] = CoreAnalyticsEngine.normalize_position(row.get("position"))
            row["size_bb"] = size_bb
    ...
    def _aggregate(
        ...
        for (position, bucket), items in groups.items():
            ...
            flop_call = sum(
                1
                for item in items
                if bool(item["flop_call_vs_cbet"])
            )
            flop_raise = sum(
                1
                for item in items
                if bool(item["flop_raise_vs_cbet"])
            )
            ...
            turn_call = sum(
                1
                for item in items
                if bool(item["turn_call_vs_barrel"])
            )
            turn_raise = sum(
                1
                for item in items
                if bool(item["turn_raise_vs_barrel"])
            )
            ...
            river_call = sum(
                1
                for item in items
                if bool(item["river_call_vs_barrel"])
            )
            river_raise = sum(
                1
                for item in items
                if bool(item["river_raise_vs_barrel"])
            )
            ...
            row = {
                ...
                "flop_call_vs_cbet": self._pct(
                    flop_call,
                    flop_opp,
                ),
                "flop_raise_vs_cbet": self._pct(
                    flop_raise,
                    flop_opp,
                ),
                ...
                "turn_call_vs_barrel": self._pct(
                    turn_call,
                    turn_opp,
                ),
                "turn_raise_vs_barrel": self._pct(
                    turn_raise,
                    turn_opp,
                ),
                ...
                "river_call_vs_barrel": self._pct(
                    river_call,
                    river_opp,
                ),
                "river_raise_vs_barrel": self._pct(
                    river_raise,
                    river_opp,
                ),
            ...
            row["response_tree"] = self._response_tree(
                row
            )
            row["break_street"] = self._break_street(
                row
            )
            row["ev_score"] = self._ev_score(
                row
            )
    ...
    def _response_tree(
        ...
        return (
            f"PF F{float(row['pool_fold_preflop']):.0f}"
            f"/C{float(row['pool_call_preflop']):.0f}"
            f"/3B{float(row['pool_3bet_preflop']):.0f}"
            f" → F F{float(row['flop_fold_vs_cbet']):.0f}"
            f"/C{float(row['flop_call_vs_cbet']):.0f}"
            f"/R{float(row['flop_raise_vs_cbet']):.0f}"
            f" → T F{float(row['turn_fold_vs_barrel']):.0f}"
            f"/C{float(row['turn_call_vs_barrel']):.0f}"
            f"/R{float(row['turn_raise_vs_barrel']):.0f}"
            f" → R F{float(row['river_fold_vs_barrel']):.0f}"
            f"/C{float(row['river_call_vs_barrel']):.0f}"
            f"/R{float(row['river_raise_vs_barrel']):.0f}"
        )
    ...
    def _break_street(
        ...
        candidates = [
            ("Preflop", float(row["pool_fold_preflop"])),
            ("Flop", float(row["flop_fold_vs_cbet"])),
            ("Turn", float(row["turn_fold_vs_barrel"])),
            ("River", float(row["river_fold_vs_barrel"])),
        ]
        ...
        return f"{street} ({value:.1f}%)"

    def _ev_score(
        self,
        row: dict[str, Any],
    ) -> float:
        preflop_fold = float(row["pool_fold_preflop"])
        three_bet_avoidance = 100.0 - float(
            row["pool_3bet_preflop"]
        )
        flop_fold = float(row["flop_fold_vs_cbet"])
        turn_fold = float(row["turn_fold_vs_barrel"])
        river_fold = float(row["river_fold_vs_barrel"])

        raw = (
            preflop_fold * 0.30
            + three_bet_avoidance * 0.20
            + flop_fold * 0.22
            + turn_fold * 0.16
            + river_fold * 0.12
        )
    ...
    def _summary(
        ...
        return (
            ...
            f"(EV {best['ev_score']:.0f}/100, "
            f"Response {best['response_score']:.0f}/100). "
            f"En çok kopuş: {best['break_street']}. "
