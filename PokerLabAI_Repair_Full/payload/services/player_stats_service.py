...
class PlayerStatsService:
    ...
    def get_players(
        ...
        identity_expr = (
            "COALESCE(pa.alias_name, hp.player_name)"
            if use_aliases
            else "hp.player_name"
        )
        ...
        query = f"""
            WITH filtered_players AS (
                SELECT
                    hp.hand_id,
                    hp.player_name AS raw_player_name,
                    {identity_expr} AS player_name,
                    hp.position
            ...
            player_hands AS (
                SELECT
                    player_name,
                    COUNT(*) AS hands,
                    COUNT(DISTINCT raw_player_name) AS merged_nicks
            ...
            preflop_flags AS (
                ...
                LEFT JOIN actions a
                 ...
                 AND a.player_name = fp.raw_player_name
                GROUP BY fp.player_name, fp.hand_id
            ),
            ...
            three_bet_flags AS (
                ...
                LEFT JOIN raise_order ro
                 ...
                 AND ro.player_name = fp.raw_player_name
                GROUP BY fp.player_name, fp.hand_id
            ),
            ...
            postflop_flags AS (
                SELECT
                    ...
                    MAX(
                        CASE
                            WHEN pfr.player_name = fp.raw_player_name
                            ...
                            WHEN pfr.player_name = fp.raw_player_name
                             ...
                             AND EXISTS (
                                ...
                                WHERE fa.hand_id = fp.hand_id
                                  AND fa.player_name = fp.raw_player_name
                                  AND fa.street = 'FLOP'
                                  AND fa.action = 'BET'
                             )
                            THEN 1 ELSE 0
                            ...
                            WHEN pfr.player_name = fp.raw_player_name
                             ...
                             AND EXISTS (
                                ...
                                WHERE ta.hand_id = fp.hand_id
                                  AND ta.player_name = fp.raw_player_name
                                  AND ta.street = 'TURN'
                                  AND ta.action = 'BET'
                             )
                            THEN 1 ELSE 0
        ...
        return [
            {
                ...
                "merged_nicks": int(row[2] or 1),
                "vpip": float(row[3] or 0.0),
                "pfr": float(row[4] or 0.0),
                "three_bet": float(row[5] or 0.0),
                "flop_cbet": self._nullable_float(row[6]),
                "flop_cbet_made": int(row[7] or 0),
                "flop_cbet_opp": int(row[8] or 0),
                "turn_barrel": self._nullable_float(row[9]),
                "turn_barrel_made": int(row[10] or 0),
                "turn_barrel_opp": int(row[11] or 0),
                "river_barrel": self._nullable_float(row[12]),
                "river_barrel_made": int(row[13] or 0),
                "river_barrel_opp": int(row[14] or 0),
            }
            for row in rows
        ]
