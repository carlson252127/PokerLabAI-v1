...
class BotDNAService:
    ...
    def build_dna(
        ...
        for key, label, category in [
            ("vpip", "VPIP", "Preflop"),
            ("pfr", "PFR", "Preflop"),
            ("flop_cbet", "Flop CBet", "Flop"),
            ("turn_barrel", "Turn Barrel", "Turn"),
            ("river_barrel", "River Barrel", "River"),
    ...
    def _build_dimensions(
        ...
        turn_pressure = self._clamp_score(
            50.0
            + delta("turn_barrel") * 3.0
        )

        river_pressure = self._clamp_score(
            50.0
            + delta("river_barrel") * 2.5
        )
    ...
    def _build_exploits(
        ...
        turn_delta = metric_map.get(
            "turn_barrel",
            {},
        ).get("delta", 0.0)

        river_delta = metric_map.get(
            "river_barrel",
            {},
        ).get("delta", 0.0)

        flop_delta = metric_map.get(
            "flop_cbet",
            {},
        ).get("delta", 0.0)
