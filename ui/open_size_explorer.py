...
class OpenSizeExplorer(QWidget):
    COLUMNS = [
        ...
        ("fold_to_three_bet", "Fold 3Bet"),
        ("pool_fold_to_three_bet", "Pool F3B"),
        ("delta_fold_to_three_bet", "Δ F3B"),
        ("fold_to_three_bet_sample", "F3B Smp"),

        ("flop_cbet_ip", "Flop CBet IP"),
        ("pool_flop_cbet_ip", "Pool FCB IP"),
        ("delta_flop_cbet_ip", "Δ FCB IP"),
        ("flop_cbet_ip_sample", "FCB IP Smp"),

        ("flop_cbet_oop", "Flop CBet OOP"),
        ("pool_flop_cbet_oop", "Pool FCB OOP"),
        ("delta_flop_cbet_oop", "Δ FCB OOP"),
        ("flop_cbet_oop_sample", "FCB OOP Smp"),

        ("turn_barrel_ip", "Turn Barrel IP"),
        ("pool_turn_barrel_ip", "Pool TB IP"),
        ("delta_turn_barrel_ip", "Δ TB IP"),
        ("turn_barrel_ip_sample", "TB IP Smp"),

        ("turn_barrel_oop", "Turn Barrel OOP"),
        ("pool_turn_barrel_oop", "Pool TB OOP"),
        ("delta_turn_barrel_oop", "Δ TB OOP"),
        ("turn_barrel_oop_sample", "TB OOP Smp"),

        ("river_barrel_ip", "River Barrel IP"),
        ("pool_river_barrel_ip", "Pool RB IP"),
        ("delta_river_barrel_ip", "Δ RB IP"),
        ("river_barrel_ip_sample", "RB IP Smp"),

        ("river_barrel_oop", "River Barrel OOP"),
        ("pool_river_barrel_oop", "Pool RB OOP"),
        ("delta_river_barrel_oop", "Δ RB OOP"),
        ("river_barrel_oop_sample", "RB OOP Smp"),

        ("wwsf", "WWSF"),
        ("pool_wwsf", "Pool WWSF"),
        ("delta_wwsf", "Δ WWSF"),
        ("wwsf_sample", "WWSF Smp"),
        ...
        ("transition_summary", "Size Transition"),
        ("confidence", "Güven"),
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Bot Open Size Explorer v5")
        title.setObjectName("PageTitle")

        subtitle = QLabel(
            "Open size gruplarını strength score, strategy tags ve exploitability ile "
            "ve otomatik exploit yorumlarıyla karşılaştırır."
        )
        ...
        self.best_size_card = self._card(
            "Best Size",
            "—",
        )
        self.exploitability_card = self._card(
            "Exploitability",
            "0",
        )
        ...
        self.position_summary_label.setObjectName(
            "OpenSizeCompare"
        )
        self.position_summary_label.setWordWrap(True)
        self.position_summary_label.hide()
        root.addWidget(self.position_summary_label)
        ...
        if rows:
            ...
            strongest = max(
                ...
                key=lambda row: float(
                    row.get("strength_score") or 0.0
                ),
            )
            self.best_size_card.value_label.setText(
                f"{strongest['position']} "
                f"{strongest['size_bucket']} "
                f"{float(strongest.get('strength_score') or 0.0):.0f}"
            )
            self.exploitability_card.value_label.setText(
                f"{float(strongest.get('exploitability') or 0.0):.0f}"
            )
        else:
            self.pattern_card.value_label.setText(
                "—"
        ...
        if position_summary:
            summary_text = " | ".join(
                (
                    f"{row['position']}: "
                    f"{row['most_common']} "
                    f"(ort. {row['avg_size_bb']:.2f}x)"
                )
                for row in position_summary
            )
            self.position_summary_label.setText(
                "Pozisyon Özeti — " + summary_text
            )
            self.position_summary_label.show()
        else:
            self.position_summary_label.hide()
    ...
    def _fill_table(
        ...
        percentage_keys = {
            ...
            "turn_barrel_oop",
            "pool_turn_barrel_oop",
            "delta_turn_barrel_oop",
            "river_barrel_ip",
            "pool_river_barrel_ip",
            "delta_river_barrel_ip",
            "river_barrel_oop",
            "pool_river_barrel_oop",
