...
class ResponseExplorer(QWidget):
    COLUMNS = [
        ...
        ("pool_fold_preflop", "PF Fold"),
        ("pool_call_preflop", "PF Call"),
        ("pool_3bet_preflop", "PF 3Bet"),
        ("preflop_sample", "PF Smp"),

        ("flop_fold_vs_cbet", "Flop Fold"),
        ("flop_call_vs_cbet", "Flop Call"),
        ("flop_raise_vs_cbet", "Flop Raise"),
        ("flop_sample", "Flop Smp"),

        ("turn_fold_vs_barrel", "Turn Fold"),
        ("turn_call_vs_barrel", "Turn Call"),
        ("turn_raise_vs_barrel", "Turn Raise"),
        ("turn_sample", "Turn Smp"),

        ("river_fold_vs_barrel", "River Fold"),
        ("river_call_vs_barrel", "River Call"),
        ("river_raise_vs_barrel", "River Raise"),
        ("river_sample", "River Smp"),

        ("break_street", "Kopuş Sokağı"),
        ("ev_score", "Exploit EV"),
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("Pool Response Explorer v2")
        title.setObjectName("PageTitle")

        subtitle = QLabel(
            "Open size gruplarına karşı poolun tüm response tree'sini "
            "ve sample-weighted exploit EV skorunu ölçer."
        )
    ...
    def _analysis_finished(
        ...
        if best:
            ...
            self.ev_card.value_label.setText(
                f"{float(best.get('ev_score') or 0):.0f}"
            )
            ...
            self.best_card.value_label.setText("—")
            self.ev_card.value_label.setText("0")
